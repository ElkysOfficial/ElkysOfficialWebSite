-- Sub-step C / Etapa 4 alinhamento arquitetural.
--
-- Antes: aprovar uma proposta criava apenas project shell + contract draft.
-- O admin precisava abrir o projeto e configurar parcelas/charges manualmente.
-- O fluxo arquitetural diz "Aprovado → cria automaticamente: ...Faturas",
-- entao falta a peca que fecha esse loop.
--
-- Esta migration:
--   1) Adiciona `billing_config jsonb` em proposals para carregar a
--      configuracao de cobranca (modelo de pagamento, datas das parcelas,
--      mensalidade opcional). Default '{}'.
--   2) Reescreve approve_proposal_to_project para, quando billing_config
--      tiver dados validos, criar tambem project_installments + charges
--      (entrada + entrega) + opcional project_subscription com charges
--      recorrentes — tudo na mesma transacao.
--
-- Backward-compat: propostas existentes ficam com billing_config = '{}'
-- e a aprovacao continua criando apenas project shell + contract (mesmo
-- comportamento anterior). Novas propostas criadas pela UI atualizada
-- carregam billing_config preenchido e geram faturas automaticamente.

-- ────────────────────────────────────────────────────────────────────
-- 1) Coluna billing_config
-- ────────────────────────────────────────────────────────────────────

ALTER TABLE public.proposals
  ADD COLUMN IF NOT EXISTS billing_config jsonb NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.proposals.billing_config IS
  'Config de cobranca para gerar faturas no momento da aprovacao.
Schema esperado:
{
  "payment_model": "50_50" | "a_vista" | "personalizado",
  "entry_percentage": numeric,
  "entry_due_date": "YYYY-MM-DD",
  "delivery_due_date": "YYYY-MM-DD",
  "subscription": null | {
    "label": text,
    "amount": numeric,
    "due_day": int,
...
Vazio ({}) → aprovacao cria apenas project shell + contract draft (modo legado).';

-- ────────────────────────────────────────────────────────────────────
-- 2) Substitui approve_proposal_to_project
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Resolver client_id (auto-converter lead se necessario).
  ...
  -- 3. Installments + charges (so se billing_config tiver dados validos)
  ...
  THEN
    ...
    -- Entry percentage segue payment_model:
    --   '50_50' → 50/50
    --   'a_vista' → 100/0
    --   'personalizado' → usa entry_percentage do JSONB (default 50)
  ...
  IF v_subscription IS NOT NULL AND jsonb_typeof(v_subscription) = 'object' THEN
    ...
    IF v_sub_amount IS NOT NULL AND v_sub_amount > 0
      ...
      -- Primeira charge agendada da subscription (proximo dueDay >= today).
      -- Geracao completa de N meses fica para o sync manual do Financeiro
      -- (botao "Sincronizar mensalidades") — mesma decisao do Step P-001.
      INSERT INTO public.charges (
  ...
  -- 6. Marcar proposta como aprovada (idempotente).
  ...
  INSERT INTO public.timeline_events (
    ...
    format('Proposta "%s" aprovada. Projeto, contrato e cobrancas iniciais foram criados.', v_proposal.title),
    'ambos',
    'proposals',
    p_proposal_id,
    auth.uid(),
...
COMMENT ON FUNCTION public.approve_proposal_to_project(uuid) IS
  'Aprova proposta atomicamente: converte lead em cliente se necessario, cria projeto + contract draft + (se billing_config preenchido) installments + charges + opcional subscription + primeira charge recorrente. Tudo em uma transacao.';
